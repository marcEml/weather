...
const ToggleChartSwitch: React.FC<IToggleSwitchProps> = (props) => {
  ...
  return (
    ...
    >
      {toggled && <span className="on">O</span>}
      {!toggled && <span className="off">N</span>}
      <Slider style={{ transform: toggled ? ' translateX(28px)' : ' translateX(0px)' }}></Slider>
    </Switch>
  );
};

export default ToggleChartSwitch;
